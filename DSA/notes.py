...
\n\n# Number of Islands\nclass Solution:
    def numIslands(self, grid):
        ...
        islands = 0
        def dfs(r, c):
             if (r not in range(rows) or c not in range(cols) or grid[r][c] == '0' or (r,c) in visited): return
             visited.add((r, c))
...
\n\n# Word Search II\nclass TrieNode:
    def __init__(self): self.children = {}; self.isWord = False; self.refs = 0
    def addWord(self, word):
        cur = self
        cur.refs += 1
        for c in word:
...
class MedianFinder:
    ...
    def addNum(self, num):
        if self.large and num > self.large[0]: heapq.heappush(self.large, num)
        else: heapq.heappush(self.small, -1 * num)
